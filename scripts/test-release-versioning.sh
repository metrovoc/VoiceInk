...
marketing_occurrences="$(grep -c 'MARKETING_VERSION = 26.7.0;' "$project_file")"
build_occurrences="$(grep -c 'CURRENT_PROJECT_VERSION = 26007000;' "$project_file")"

if [ "$marketing_occurrences" -ne "$expected_marketing_occurrences" ]; then
  echo "Expected both VoiceInk app configurations to use MARKETING_VERSION 26.7.0" >&2
  exit 1
fi

if [ "$build_occurrences" -ne "$expected_build_occurrences" ]; then
  echo "Expected both VoiceInk app configurations to use CURRENT_PROJECT_VERSION 26007000" >&2
  exit 1
fi
